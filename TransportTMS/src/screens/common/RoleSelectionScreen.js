...
const RoleSelectionScreen = () => {
  ...
  const handleSelectRole = (role) => { dispatch(selectRole(role)); };
  ...
  return (
    <View style={styles.container}>
      ...
      {roles.map((r) => (
        <TouchableOpacity key={r.key} style={[styles.card, { borderLeftColor: r.color }]} onPress={() => handleSelectRole(r.key)} activeOpacity={0.75}>
